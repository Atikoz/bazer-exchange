 ...
 * Отправка в БЧ транзакции делегирования и возврат ее хэша
 * @param wallet объект с кошельком отправителя
 * @param amount сумма в uARTR
 * @param nodeUrl url ноды
 * @return {Promise<void>}
 */
async function delegate(wallet, amount, nodeUrl) {
    let txData = ArtrWallet.txFactory.wrap(ArtrWallet.txFactory.delegate(wallet._getSigner(), wallet.address, amount, '90000000'));
    const response = await fetch(nodeUrl + '/cosmos/tx/v1beta1/txs', {
        method: 'POST',
        body: JSON.stringify(txData)
    })
