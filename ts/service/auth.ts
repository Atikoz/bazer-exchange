import createArteryWallet from "../function/createWallet/createArteryWallet";
import createDecimalWallet from "../function/createWallet/createDecimalWallet";
import CreateMinePlexWallet from "../function/createWallet/createMinePlexWallet";
import createMinterWallet from "../function/createWallet/createMinterWallet";
import CreateMpxXfiWallet from "../function/createWallet/createMpxXfiWallet";
import CreateUsdtWallet from "../function/createWallet/createUsdtWallet";
import BalanceUserModel from "../model/modelBalance";
import ProfitPoolModel from "../model/liquidityPool/modelProfitPool";
import UserModel from "../model/modelUser";
import WalletUserModel from "../model/modelWallet";
import sendMessage from "../../helpers/tgFunction";

//код 10 - ошибка создания кошелька Artery



class AuthenticationService {
  public async Authentication(userId: number): Promise<string | void> {
    try {
      if (!await UserModel.findOne({ id: userId })) {
        const createDelWallet = await createDecimalWallet();
        if (createDelWallet.status != 'ok') return this.Authentication(userId);

        const createUsdt = await CreateUsdtWallet();

        const createMinePlex = await CreateMinePlexWallet(createDelWallet.del.mnemonics);

        const createMpxXfi = await CreateMpxXfiWallet(createDelWallet.del.mnemonics);

        const createArtery = await createArteryWallet.createUserArteryWallet(createDelWallet.del.mnemonics);
        if (!createArtery) return sendMessage(userId, 'Ошибка при регистрации код 10, сообщите его администрации для решения проблемы.');

        const createMinter = createMinterWallet(createDelWallet.del.mnemonics);


        await UserModel.create({
          id: userId,
          status: 0,
          mail: null,
          lang: 'eng'
        });

        await ProfitPoolModel.create({
          id: userId,
          profit: 0
        });

        await WalletUserModel.create({
          id: userId,
          mnemonics: createDelWallet.del.mnemonics,
          del: {
            address: createDelWallet.del.address,
          },
          usdt: {
            address: createUsdt.address,
            privateKey: createUsdt.privateKey
          },
          minePlex: {
            address: createMinePlex.data.keys.pkh,
            sk: createMinePlex.data.keys.sk,
            pk: createMinePlex.data.keys.pk
          },
          mpxXfi: {
            address: createMpxXfi.data.account.address
          },
          artery: {
            address: createArtery
          },
          minter: {
            address: createMinter.address,
            privateKey: createMinter.privateKey
          }
        });

        await BalanceUserModel.create({
          id: userId,
          main: {
            usdt: 0,
            mine: 0,
            plex: 0,
            mpx: 0,
            xfi: 0,
            artery: 0,
            bip: 0,
            hub: 0,
            monsterhub: 0,
            bnb: 0,
            usdtbsc: 0,
            bipkakaxa: 0,
            cashbsc: 0,
            minterBazercoin: 0,
            ruble: 0,
            cashback: 0,
            del: 0,
            ddao: 0,
            delkakaxa: 0,
            converter: 0,
            pro: 0,
            dar: 0,
            sbt: 0,
            reboot: 0,
            makarovsky: 0,
            btt: 0,
            dixwell: 0,
            avt: 0,
            kharat: 0,
            byacademy: 0,
            patrick: 0,
            itcoin: 0,
            messege: 0,
            rrunion: 0,
            vegvisir: 0,
            fbworld: 0,
            dcschool: 0,
            comcoin: 0,
            mintcandy: 0,
            sirius: 0,
            cgttoken: 0,
            genesis: 0,
            taxicoin: 0,
            prosmm: 0,
            sharafi: 0,
            safecoin: 0,
            dtradecoin: 0,
            izicoin: 0,
            gzacademy: 0,
            workout: 0,
            zaruba: 0,
            magnetar: 0,
            candypop: 0,
            randomx: 0,
            ekology: 0,
            emelyanov: 0,
            belymag: 0,
            doorhan: 0,
            lakshmi: 0,
            ryabinin: 0,
            related: 0,
            monopoly: 0,
            baroncoin: 0,
            nashidela: 0,
            irmacoin: 0,
            maritime: 0,
            business: 0,
            randice: 0,
            alleluia: 0,
            hosanna: 0,
            cbgrewards: 0,
            novoselka: 0,
            monkeyclub: 0,
            grandpay: 0,
            magnate: 0,
            crypton: 0,
            iloveyou: 0,
            bazercoin: 0,
            bazerusd: 0,
          },
          hold: {
            usdt: 0,
            mine: 0,
            plex: 0,
            mpx: 0,
            xfi: 0,
            artery: 0,
            bip: 0,
            hub: 0,
            monsterhub: 0,
            bnb: 0,
            usdtbsc: 0,
            bipkakaxa: 0,
            cashbsc: 0,
            minterBazercoin: 0,
            ruble: 0,
            cashback: 0,
            del: 0,
            ddao: 0,
            delkakaxa: 0,
            converter: 0,
            pro: 0,
            dar: 0,
            sbt: 0,
            reboot: 0,
            makarovsky: 0,
            btt: 0,
            dixwell: 0,
            avt: 0,
            kharat: 0,
            byacademy: 0,
            patrick: 0,
            itcoin: 0,
            messege: 0,
            rrunion: 0,
            vegvisir: 0,
            fbworld: 0,
            dcschool: 0,
            comcoin: 0,
            mintcandy: 0,
            sirius: 0,
            cgttoken: 0,
            genesis: 0,
            taxicoin: 0,
            prosmm: 0,
            sharafi: 0,
            safecoin: 0,
            dtradecoin: 0,
            izicoin: 0,
            gzacademy: 0,
            workout: 0,
            zaruba: 0,
            magnetar: 0,
            candypop: 0,
            randomx: 0,
            ekology: 0,
            emelyanov: 0,
            belymag: 0,
            doorhan: 0,
            lakshmi: 0,
            ryabinin: 0,
            related: 0,
            monopoly: 0,
            baroncoin: 0,
            nashidela: 0,
            irmacoin: 0,
            maritime: 0,
            business: 0,
            randice: 0,
            alleluia: 0,
            hosanna: 0,
            cbgrewards: 0,
            novoselka: 0,
            monkeyclub: 0,
            grandpay: 0,
            magnate: 0,
            crypton: 0,
            iloveyou: 0,
            bazercoin: 0,
            bazerusd: 0
          }
        });

        return 'ok';

      } else return;
    } catch (error) {
      console.error(error)
    }
  }
}

export default new AuthenticationService;