...
const getMinterCoinRate = async (): Promise<any> => {
  try {
    const rateHub = {
      uah: 229.06,
      try: 189.67,
    ...
    for (const currency of Object.keys(convertId)) {
      const configResponce = {
        method: 'get',
        url: `https://pro-api.coinmarketcap.com/v1/cryptocurrency/category?id=604f2762ebccdd50cd175fcc&convert_id=${convertId[currency]}`,
        headers: {
          'X-CMC_PRO_API_KEY': config.apiKeyCoinMarketCap
        }
      };

      const response = await axios.request(configResponce);
      const dataBip = response.data.data.coins.filter((el: any) => el.id === 4957);
...
const getAllCoinRate = async (): Promise<any> => {
  try {
    ...
    const rateToCurr: any = {
      rub: {
        ...
        bipkakaxa: priceMinterCoin.bipkakxa.rub,
      ...
      uah: {
        ...
        bipkakaxa: priceMinterCoin.bipkakxa.uah,
      ...
      try: {
        ...
        bipkakaxa: priceMinterCoin.bipkakxa.try,
    ...
    for (const currency in rateToCurr) {
      ...
      for (const symbol in currencyObject) {
        ...
        const foundItem = rateDecimalCoin.find((item: any) => item.symbol === symbol);
